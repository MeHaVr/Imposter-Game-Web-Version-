...
function SettingsMenu({
  ...
  const addWordsFromSet = async (id: string) => {
    try {
      ...
      setErrorMsg("Fehler beim Laden des Word Sets.");
      setIsError(true);
      setErrorType("error");
    }
  };
  ...
  const redeemPrivatCode = async () => {
    ...
    try {
      ...
      setPrivatCode("");
      setErrorMsg("✅ Privat-Set erfolgreich eingelöst!");
      setIsError(true);
      setErrorType("success");
    } catch (err: any) {
      console.error("Redeem error:", err);
      const errorMsg = err?.response?.data?.error ?? "Einlösen fehlgeschlagen.";
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className="px-5 pt-5 pb-6 space-y-5 overflow-y-auto overscroll-contain flex-1 [scrollbar-gutter:stable]"
          style={{
          ...
          <div className="rounded-xl border border-white/10 bg-white/5 p-4">
            ...
            <div className="space-y-2">
              <input
                ...
                className="w-full rounded-lg bg-white/10 px-4 py-2.5 text-sm text-white placeholder:text-white/40 outline-none transition focus:bg-white/15 focus:ring-2 focus:ring-blue-500/60"
              />
          ...
          <div className="rounded-xl border border-white/10 bg-white/5 p-4">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className={`w-full rounded-lg px-4 py-3 text-sm text-white bg-white/10 placeholder:text-white/40 outline-none transition focus:bg-white/15 focus:ring-2 ${redeemError ? "ring-2 ring-red-500/50 focus:ring-red-500/60" : "focus:ring-blue-500/60"}`}
              />
